...
SELECT VictimID AS PersonID, 'Victim' AS Role, Age
FROM Victim
WHERE Age IS NOT NULL
UNION
SELECT SuspectID AS PersonID, 'Suspect' AS Role, Age
FROM Suspect
WHERE Age IS NOT NULL
ORDER BY Age DESC;
...
FROM Crime
RIGHT JOIN Victim ON Crime.CrimeID = Victim.CrimeID
RIGHT JOIN Suspect ON Crime.CrimeID = Suspect.CrimeID; 


-- 15.Find incidents where the suspect is older than any victim.

SELECT Crime.CrimeID, Crime.IncidentType, Suspect.SuspectName, Suspect.Age 
FROM Crime
JOIN Suspect ON Crime.CrimeID = Suspect.CrimeID
...
AND c2.IncidentType NOT IN ('Homicide', 'Robbery')
  );
